...
export class BillsComponent extends BaseComponent<any> {
  ...
  async saveBulkPayment() {
    ...
    if (this.bulkPayment.valid && amt != 0 && amt != '') {
      ...
      for( const ptn of this.patients || []){
  ...
  printInvoice(){
    ...
    if(this.selected_partner?.id){
      ...
      this.patients?.forEach(async (ptn: any) => {
        if(ptn.invoice?.total_paid == '0.00'){
      ...
      if(patients.length != 0 && this.selected_partner?.id){
        ...
        const companyModel = {
          ...
          grn_number: this.grnNum,
          invoice_number: this.invoiceNum
        }

        this.updateCompanyAndSendMail(companyModel, model);

  
      }else{

        this.invoiceLoading = false ;
        this.alertService.showInfo("No Unpaid Patients.")
      }
    }else{
  ...
  updateCompanyAndSendMail(companyModel: any, model: any){
    this.subsink.sink = this.endPoint.updateCompany(companyModel).subscribe((res: any)=>{

...
export class AddpatientComponent extends BaseComponent<AddPatientsModel> {
  ...
  @Input() is_estimation : boolean = false ;
  ...
  // capitalize(e: any, type: any) {
  //   e = e.replace(/[^a-zA-Z.\s]/g, '')
  //   this.baseForm.get(type)?.setValue(e.replace(/[^a-zA-Z.\s]/g, '').toUpperCase());
  // }
  ...
  //   this.baseForm.get('name')?.setValue(cleanedInput.toUpperCase());
  // }
  ...
  deleteTest(index: number, test: any) {
    if (test.package) {
      ...
      this.baseForm.get('lab_tests')?.setValue(this.baseForm.value.lab_tests.filter((obj: any) => obj.LabGlobalTestId !== test.id));
  ...
  private getPatientModel(pay = false): AddPatientsModel {
    if (pay) {
      ...
      const model: any = {
        ...
        payment_for: 3,
        payments: [
          { pay_mode: null, paid_amount: 0 }
        ],
      };

      if(this.manualDateTime){
        model.added_on_time = this.timeSrvc.getAddedOnTime(model.added_on_time)
      }else{
  ...
  printQuotation(){
    if(this.tests_included.length != 0 || this.package_included.length !=0){
      const model = {
        test_ids: this.tests_included,
        package_ids: this.package_included,
        printed_by : this.staffId
      }
      this.subsink.sink = this.endPoint.postNPrintQuotation(model).subscribe((res: any)=>{

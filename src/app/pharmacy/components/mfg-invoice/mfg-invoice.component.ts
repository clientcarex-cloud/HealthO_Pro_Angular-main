...
@Component({
  selector: 'app-mfg-invoice',
  templateUrl: './mfg-invoice.component.html',
})

export class MfgInvoiceComponent extends BaseComponent<any> {
  ...
  @ViewChild('addInvoice') addInvoice: any ;
  ...
  openAddInvoice(){
    this.selected_item = null ;
    this.openModal(this.addInvoice, { size: 'xl', centered: true })
  }
  ...
  getData() {
    this.subsink.sink = this.endPoint.getPharmaStock(
      this.page_size, this.page_number,
      this.query, this.from_date, this.to_date, '', this.sort
    ).subscribe((data: any) => {

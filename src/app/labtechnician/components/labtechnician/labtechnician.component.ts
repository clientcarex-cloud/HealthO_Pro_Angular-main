...
import { LabTechnicianEndpoint } from '../../endpoints/labtechnician.endpoint';
...
export class LabtechnicianComponent extends BaseComponent<LabTechnician> {
  ...
  setStatus(buttonId: string) {
    ...
    switch (this.activeButton) {
      ...
      case 'processing':
        ...
        break;
      case 'emergency':
        this.statusQuery = "&status_id=11,15,19,20";
        break;
      case 'completed':
      ...
      case 'reported':
        ...
        // Optionally handle other statements related to 'reported'
        break;
      default:
  ...
  override ngOnInit(): void {
    ...
    this.subsink.sink = this.patientEndpoint.getDepartments().subscribe((data: any) => {
      ...
      if (!this.is_sourcing) this.getData();
    })
  ...
  printTechnicianReport(model: any, printAccess: any, show_header: boolean, download: boolean) {
    if (printAccess) {
      this.subsink.sink = this.endPoint.printReport(model).subscribe((response: any) => {
        ...
        if (!download) {
          ...
          // if(!response?.is_only_fixed){
          //   this.printSrvc.previewIframe(response.html_content, response.header, this.settings, this.bg_img, false);
          // }else{
          //   this.printSrvc.printTabular(response.header, response.html_content, response.footer, show_header, this.settings)
          // }
  ...
  sendCreateReportStatus(patient: any, newTemplate: any) {
    ...
    if (patient.phlebotomist?.is_received) {
      ...
      this.subsink.sink = this.endPoint.getReportParams(model).subscribe((response: any) => {
        ...
        this.spinner.hide() ;
        if(newTemplate && this.patient?.id){
          this.loadPatientList(this.patient.id).then((res:any)=>{
            this.patient = res ;
          });
        }
  ...
  wordToFixedReport(devRich: any, test: any, access: boolean = false, newTemplate: any){

    if(newTemplate.report_type == 2){
      devRich.changeTestTemplate(newTemplate);
    }else{
      this.deleteFixedTestReport(test, access, newTemplate)
    }
  }
            ...
            if (this.due_print || this.patient.LabPatientTestID.patient.total_due == 0) {
              ...
              if (event.save) {
                ...
                if (this.print_permissons && this.print_permissons?.technician_print) {
                  this.printTechnicianReport(this.getPrintReportModel(), this.canGivePrint, this.settings.display_letterhead, false);
                } else {
                  this.getData();
                }
              }
  ...
  printTestReport(data: any, letterhead: boolean, download: boolean){
    ...
    if (this.due_print || data.LabPatientTestID.patient.total_due == 0) {
      this.printTechnicianReport(details, this.canGivePrint, letterhead, download)
    } else {
      this.alertService.showInfo(`You Don't Have Access To Due ${download ? 'download': 'print'}.`)
    }
  }

  printReportForEdit(data: any, access: any, msg: any, letterhead: boolean, download: boolean) {

...
export class AddPrescriptionComponent extends BaseComponent<any> {
  ...
  getPrescription(){
    if(this.patient){
      ...
      this.subsink.sink = this.hospitalEndpoint.getPatientPrescription(this.patient.patient.id, this.consultation?.id).subscribe((res: any)=>{
      ...
      this.title = `${this.patient.patient.title}${this.patient.patient.name}`
    }
  }

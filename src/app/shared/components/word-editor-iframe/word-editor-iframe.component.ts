...
@Component({
	selector: 'app-word-editor-iframe',
	templateUrl: './word-editor-iframe.component.html',
	styleUrl: './word-editor-iframe.component.scss'
})


export class WordEditorIframeComponent extends BaseComponent<any> {


	// FONT_SIZE_SELECT
	config: EditorConfig = {
		placeholder: 'Type something...',
		buttons: [UNDO_BUTTON, REDO_BUTTON, SEPARATOR,
	...
	override ngOnDestroy(): void {
		...
		this.docLoading = false;
		this.documentEventsRemove();
	}

	documentEventsRemove() {
	...
	documentEventsAdd() {
		this.keydownListener = (event: KeyboardEvent) => {
			if (event.ctrlKey && event.key === ']') {
				...
				if (window.location.pathname == '/setup/tweaks' && this.opened) {
					event.preventDefault();
					insertTab();
				}
	...
	setContent() {
		this.st_area_editor = `
    	<div style="display: flex; flex-direction: row; flex-wrap: nowrap; justify-content: space-between; overflow: hidden; height: 100vh;">
			<div contenteditable="false" id="previewofreport" style="user-select: none;background-color: gray; display: block; justify-content: center; padding: 10px 0 0 15px; position: sticky; top: 0; height: 100vh;">
				<iframe id="preview_iframe" style="height: 100%; width: 69mm;"></iframe>
			</div>
			<div class="bg-white border overflow-scroll wordreportcontent" id="wordreportcontent" style="width: 221mm;padding: 15px;overflow: scroll !important; height: 70vh">
				${this.autoCloseTags(DOMPurifyI.sanitize(this.content))}
			</div>
		<div>
		`

		this.st_area_editor = DOMPurifyI.sanitize(this.content)
	...
	openPreview() {
		let content = this.printSrvc.previewIframe(this.getContent(), this.header_to_diplay_in_every_page, this.settings, this.bg);
